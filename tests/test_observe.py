...
def test_tactile_shift_with_odd_bin(key: chex.PRNGKey) -> None:
    #     x
    #   O x←3
    # O x 2→ ←4
    env, state, _ = reset_env(key, n_tactile_bins=5, shift=36.0)
    step = jax.jit(env.step)
    act1 = jnp.zeros((10, 2)).at[2, 0].set(20).at[3:5, 1].set(20)
    while True:
        state, ts = step(state, act1)
    ...
    for _ in range(100):
        ...
        if not p2p4_ok and jnp.linalg.norm(p2 - p4) <= 2 * AGENT_RADIUS:
            assert bool(ts.encount[2, 4]), (p2, p3, p4)
            assert bool(ts.obs.collision[2, 0, 0]), (p2, p3, p4)
            assert bool(ts.obs.collision[4, 0, 0]), (p2, p3, p4)
            p2p4_ok = True
...
def test_tactile_shift_with_even_bin(key: chex.PRNGKey) -> None:
    #     x
    #   O x←3
    # O x 2→ ←4
    env, state, _ = reset_env(key, n_tactile_bins=6, shift=60.0)
    step = jax.jit(env.step)
    act1 = jnp.zeros((10, 2)).at[2, 0].set(20).at[3:5, 1].set(20)
    while True:
        state, ts = step(state, act1)
    ...
    for _ in range(100):
        ...
        if not p2p4_ok and jnp.linalg.norm(p2 - p4) <= 2 * AGENT_RADIUS:
            assert bool(ts.encount[2, 4]), (p2, p3, p4)
            assert bool(ts.obs.collision[2, 0, 0]), (p2, p3, p4)
            assert bool(ts.obs.collision[4, 0, 1]), (p2, p3, p4)
            p2p4_ok = True

        p3_to_food = jnp.linalg.norm(p3 - jnp.array([80.0, 90.0]))
        if not p3_ok and p3_to_food <= AGENT_RADIUS + FOOD_RADIUS:
            assert bool(ts.obs.collision[3, 1, 1]), (p2, p3, p4)
            p3_ok = True
